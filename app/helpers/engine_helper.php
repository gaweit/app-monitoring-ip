...
{
    $ci = &get_instance();
    $nm = $ci->db->query("SELECT nama_blok FROM t_blok WHERE kode_blok = '" . $k . "' ORDER BY kode_blok DESC LIMIT 1")->row_array();
    return $nm['nama_blok'];
}
...
{
    $ci = &get_instance();
    $nm = $ci->db->query("SELECT nama FROM t_users WHERE kode_users = '" . $k . "' ORDER BY kode_users DESC LIMIT 1")->row_array();
    if ($nm != '') {
        return $nm['nama'];
    } else {
        return 'Tidak Ada';
