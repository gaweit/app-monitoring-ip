...
class Device extends CI_Controller
{
	...
	function rute()
	{
		...
		if ($lv == '1' || $lv == '2' || $lv == '8') {
			...
			$qu = $this->model_app->view_join_wheres('t_client', 't_blok', 'kode_blok', array('kode_klient' => $kode), 'kode_klient', 'DESC');
			$rows = $qu->row_array();
	...
	public function tambah_device()
	{
		if (isset($_POST['submit'])) {
			$data = array(
				'kode_klient'    => $this->mylibrary->kdauto('t_client', 'kode_klient', 'CLIENT'),
	...
	public function ubah_device()
	{
		...
		if ($lv == '1' || $lv == '2') {
			...
			if (isset($_POST['submit'])) {
				...
				$data['row'] = $this->model_app->edit('t_client', array('kode_klient' => $id))->row_array();
				$data['serv'] = $this->model_app->view('t_blok')->result_array();
