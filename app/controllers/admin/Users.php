...
class Users extends CI_Controller
{
	...
	function indeks()
	{
		...
		$data['record'] = $this->model_app->view_ordering('t_users', 'kode_user', 'DESC');
	...
	function change_password()
	{
		if (isset($_POST['submit'])) {
			...
			$where = array('kode_user' => $this->session->userdata('id'));
	...
	function tambah_users()
	{
		if (isset($_POST['submit'])) {
			...
			if ($hasil['file_name'] == '') {
				$data = array(
					'kode_user' => $this->mylibrary->kdauto('t_users', 'kode_user', 'USR'),
	...
	function ubah_users()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$where = array('kode_user' => $this->input->post('id'));
			$q = $this->model_app->update('t_users', $data, $where);
			if ($q) {
				$this->session->set_flashdata('success', 'Data berhasil diproses!');
			...
			$data['row'] = $this->model_app->edit('t_users', array('kode_user' => $id))->row_array();
			$data['userss'] = $this->model_app->view_where_order('t_users', array('blokir' => 'N'), 'kode_user', 'DESC');
			$data['level'] = $this->model_app->view_where('t_level', array('option' => 'backend', 'aktif' => 'Y'))->result_array();
	...
	{
		$id = $this->uri->segment(3);
		$where = array('kode_user' => $id, 'blokir' => 'N');
		$data = array('blokir' => 'Y');
		$q = $this->model_app->update('t_users', $data, $where);
		if ($q) {
			$this->session->set_flashdata('success', 'Data berhasil diproses!');
	...
	{
		$id = $this->uri->segment(3);
		$where = array('kode_user' => $id, 'blokir' => 'Y');
		$data = array('blokir' => 'N');
		$q = $this->model_app->update('t_users', $data, $where);
		if ($q) {
			$this->session->set_flashdata('success', 'Data berhasil diproses!');
